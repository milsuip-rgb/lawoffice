...
export default function Admin() {
  ...
  useEffect(() => {
    // Load popups
    const savedPopups = localStorage.getItem('bj_admin_popups');
    if (savedPopups) {
      setPopups(JSON.parse(savedPopups));
    } else {
      setPopups([DEFAULT_POPUP]);
      localStorage.setItem('bj_admin_popups', JSON.stringify([DEFAULT_POPUP]));
    }

    // Load cases
    const savedCases = localStorage.getItem('bj_admin_cases');
    if (savedCases) {
      setCases(JSON.parse(savedCases));
    } else {
      setCases(initialCases);
      localStorage.setItem('bj_admin_cases', JSON.stringify(initialCases));
    }

    // Load lawyers
    const savedLawyers = localStorage.getItem('bj_admin_lawyers');
    if (savedLawyers) {
      setLawyersList(JSON.parse(savedLawyers));
    } else {
      setLawyersList(initialLawyers);
      localStorage.setItem('bj_admin_lawyers', JSON.stringify(initialLawyers));
    }

    // Load consultations
    const savedConsults = localStorage.getItem('bj_admin_consultations');
    if (savedConsults) {
      setConsultations(JSON.parse(savedConsults));
    } else {
      setConsultations(mockConsultations);
      localStorage.setItem('bj_admin_consultations', JSON.stringify(mockConsultations));
    }

    // Load reviews
    const savedReviews = localStorage.getItem('bj_admin_reviews');
    if (savedReviews) {
      setReviews(JSON.parse(savedReviews));
    } else {
      setReviews(DEFAULT_REVIEWS);
      localStorage.setItem('bj_admin_reviews', JSON.stringify(DEFAULT_REVIEWS));
    }
  }, []);

  const savePopups = (newPopups: any[]) => {
    setPopups(newPopups);
    localStorage.setItem('bj_admin_popups', JSON.stringify(newPopups));
  };

  const saveCases = (newCases: any[]) => {
    setCases(newCases);
    localStorage.setItem('bj_admin_cases', JSON.stringify(newCases));
  };

  const saveLawyers = (newLawyers: any[]) => {
    setLawyersList(newLawyers);
    localStorage.setItem('bj_admin_lawyers', JSON.stringify(newLawyers));
  };

  const saveConsultations = (newConsults: any[]) => {
    setConsultations(newConsults);
    localStorage.setItem('bj_admin_consultations', JSON.stringify(newConsults));
  };

  const saveReviews = (newReviews: any[]) => {
    setReviews(newReviews);
    localStorage.setItem('bj_admin_reviews', JSON.stringify(newReviews));
  };
  ...
  const handleSaveReview = (e: React.FormEvent) => {
    ...
    let newReviews;
    if (exists) {
      newReviews = reviews.map(r => r.id === editingReview.id ? editingReview : r);
    } else {
  ...
  const handleReviewImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setEditingReview({ ...editingReview, bg: reader.result as string });
      };
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setEditingPopup({ ...editingPopup, imageUrl: reader.result as string });
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleCaseImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setEditingCase({ ...editingCase, image: reader.result as string });
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleSaveCase = (e: React.FormEvent) => {
    ...
    let newCases;
    if (exists) {
      newCases = cases.map(c => c.id === editingCase.id ? editingCase : c);
    } else {
      newCases = [...cases, editingCase];
    }
    saveCases(newCases);
  ...
  const handleLawyerImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setEditingLawyer({ ...editingLawyer, image: reader.result as string });
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleDeleteLawyer = (id: number) => {
    if (window.confirm('정말 삭제하시겠습니까?')) {
      const newLawyers = lawyersList.filter(l => l.id !== id);
      saveLawyers(newLawyers);
    }
  };

  const handleSaveLawyer = (e: React.FormEvent) => {
    e.preventDefault();
    const exists = lawyersList.find(l => l.id === editingLawyer.id);
    let newLawyers;
    if (exists) {
      newLawyers = lawyersList.map(l => l.id === editingLawyer.id ? editingLawyer : l);
    } else {
      newLawyers = [...lawyersList, editingLawyer];
    }
    saveLawyers(newLawyers);
  ...
  const handleConfirmConsultation = (id: number) => {
    const newConsults = consultations.map(c => c.id === id ? { ...c, status: 'read' } : c);
    saveConsultations(newConsults);
  };

  const handleDeleteConsultation = (id: number) => {
    if (window.confirm('정말 삭제하시겠습니까?')) {
      const newConsults = consultations.filter(c => c.id !== id);
      saveConsultations(newConsults);
    }
  };
